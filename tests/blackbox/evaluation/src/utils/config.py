...
class Config:
    ...
    def init(self) -> None:
        ...
        if self.test_cluster_ca_data is None or self.test_cluster_ca_data == "":
            return ValueError("ERROR: Env TEST_CLUSTER_CA_DATA is missing")
        ...
        if self.test_cluster_auth_token is None or self.test_cluster_auth_token == "":
            return ValueError("ERROR: Env TEST_CLUSTER_AUTH_TOKEN is missing")
    
        self.aicore_deployment_id_gpt4 = os.environ.get("AICORE_DEPLOYMENT_ID_GPT4")
        if self.aicore_deployment_id_gpt4 is None or self.aicore_deployment_id_gpt4 == "":
            return ValueError("ERROR: Env AICORE_DEPLOYMENT_ID_GPT4 is missing")
    
        self.aicore_configuration_id_gpt4 = os.environ.get("AICORE_CONFIGURATION_ID_GPT4")
        if self.aicore_configuration_id_gpt4 is None or self.aicore_configuration_id_gpt4 == "":
            return ValueError("ERROR: Env AICORE_CONFIGURATION_ID_GPT4 is missing")

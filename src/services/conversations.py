...
def generate_questions(context: str, llm: ChatOpenAI) -> str:
    prompt = PromptTemplate(
        template=CONVERSATION_TEMPLATE,
        partial_variables={"context": context},
    )
    prompt = prompt.format_prompt(context=context)

    result = llm.invoke(prompt)

    # TODO: add output parser to extract questions from the result.
...
if __name__ == "__main__":
    ...
    questions = generate_questions(context, llm)
    print(questions)
...
async def get_relevant_context_from_k8s_cluster(message: Message, k8s_client: IK8sClient) -> str:
    ...
    if is_empty_str(namespace) and kind.lower() == CLUSTER:
        ...
        # Fetch node metrics - metrics server may not be installed (e.g. in K3d test environments)
        try:
            metrics_data = await k8s_client.list_nodes_metrics()
            metrics = yaml.dump_all(metrics_data)
        except Exception as e:
            logger.warning(f"Failed to fetch node metrics (metrics server may not be installed): {e}")
            metrics = "# Node metrics not available (metrics server not installed)\n"
    ...
    if data_sanitizer:
        try:
            ...
            logger.warning(f"Failed to sanitize context data: {e}. Returning unsanitized context.")
            # Return unsanitized context if sanitization fails
            # This ensures the user still gets cluster information even if PII detection fails

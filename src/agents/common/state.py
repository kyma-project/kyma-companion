...
class GatekeeperResponse(BaseModel):
    ...
    is_feedback: Annotated[
        bool,
        Field(
            default=False,
            description="""
                Classifies a user's query about the previous agent response as 
                either feedback (`True`) or not feedback (`False`).

                The primary determinant is **evaluation**: Does the user's query assess, judge, 
                or react to the quality, usefulness, or correctness of your last answer?
                
                **Classify as `True` (Feedback) if the query contains any of the following:**
                * **Direct Evaluation:** Uses evaluative words to judge past performance (e.g., "great," "wrong," 
                    "helpful," "confusing," "useful," "incorrect").
                * **Emotional Reaction:** Expresses satisfaction, frustration, or gratitude regarding the 
                    response (e.g., "thanks," "that helped," "this is frustrating").
                * **Implicit Assessment:** Comments on the response's effectiveness or how it compares to their 
                    needs (e.g., "That's not what I meant," "Finally, that worked," "That's closer", 
                    "this doesn't work," "this is incomplete").
                
                **Classify as `False` (Not Feedback) if the query's primary purpose is to:**
                * **Request Clarification:** Asks for more detail, explanation, or improvement of the 
                    answer (e.g., "Can you clarify more?", "elaborate more", "I need more details", 
                    "give me a shorter explanation").

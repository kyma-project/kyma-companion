...
class IInitialQuestionsHandler(Protocol):
    ...
    def fetch_relevant_data_from_k8s_cluster(self, message: Message, k8s_client: IK8sClient) -> str:
...
class InitialQuestionsHandler:
    ...
    def fetch_relevant_data_from_k8s_cluster(self, message: Message, k8s_client: IK8sClient) -> str:
        ...
        if is_empty_str(message.namespace) and message.resource_kind.lower() == "cluster":
        ...
        elif is_non_empty_str(message.namespace) and message.resource_kind.lower() == "namespace":
            logger.info("Fetching all K8s Events with warning type")
            context = yaml.dump_all(k8s_client.list_k8s_warning_events(namespace=str(message.namespace)))
